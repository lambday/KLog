...
void BaseGraphClass::SetVertexNumericAttributeList(unsigned aID, const vector<double>& aAttributeList) {
	if (aID >= mVertexSize)
		throw range_error("Setting vertex attribute for non existing vertex: " + stream_cast<string>(aID));
...
vector<string> BaseGraphClass::GetVertexSymbolicAttributeList(unsigned aID) const {
	if (aID >= mVertexSize)
		throw range_error("Getting vertex attributes for non existing vertex: " + stream_cast<string>(aID));
	return mVertexSymbolicAttributeList[aID];
}
const vector<string>& BaseGraphClass::GetVertexSymbolicAttributeListRef(unsigned aID) const {
	if (aID >= mVertexSize)
		throw range_error("Getting vertex attributes for non existing vertex: " + stream_cast<string>(aID));
	return mVertexSymbolicAttributeList[aID];
}
...
string BaseGraphClass::GetVertexSymbolicAttributeList(unsigned aID, unsigned aAttributeID) const {
	if (aID >= mVertexSize)
		throw range_error("Getting vertex attributes for non existing vertex: " + stream_cast<string>(aID));
	if (aAttributeID > mVertexSymbolicAttributeList[aID].size())
		throw range_error("Getting vertex attributes for non existing attribute id: " + stream_cast<string>(aAttributeID));
...
void BaseGraphClass::SetEdgeNumericAttributeList(unsigned aID, const vector<double>& aAttributeList) {
	if (aID >= mEdgeSize)
		throw range_error("Setting edge attribute for non existing edge: " + stream_cast<string>(aID));
