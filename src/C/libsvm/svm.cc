...
void printx(const svm_node* x) {
    const svm_node* p = x;
    while (p->index > 0) {
        printf("%d:%.2f ", p->index, p->value);
        p++;
    }
    printf("\n");
}
...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param) {
    ...
    if (param->svm_type == ONE_CLASS ||
        ...
        // for(i=0;i<nr_class;i++)// FIXME: debug
        //   printf("End training - class label[%d] = %d\n",i,model->label[i]);
        ...
        *dec_values = sum;
        ...
        for (i = 0; i < nr_class; i++)
            for (int j = i + 1; j < nr_class; j++) {
                ...
                sum -= model->rho[p];
                // printf("Prediction sum class %d vs %d =%f\n",i,j,sum);
                dec_values[p] = sum;
